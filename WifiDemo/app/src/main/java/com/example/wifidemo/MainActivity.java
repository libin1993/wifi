...
import com.example.wifidemo.adapter.WifiListAdapter;
import com.example.wifidemo.app.AppContants;
import com.example.wifidemo.bean.WifiBean;
import com.example.wifidemo.dialog.WifiLinkDialog;
import com.example.wifidemo.utils.CollectionUtils;
...
//    private WifiListAdapter adapter;
    ...
    private BaseQuickAdapter<WifiBean,BaseViewHolder> adapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        rvWifi = (RecyclerView) this.findViewById(R.id.recy_list_wifi);
    ...
    private void initRecycler() {
        ...
        adapter = new BaseQuickAdapter<WifiBean, BaseViewHolder>(R.layout.item_wifi_list,realWifiList) {
            @Override
            protected void convert(BaseViewHolder helper, WifiBean item) {
                helper.setText(R.id.tv_item_wifi_name,item.getWifiName());
                helper.setText(R.id.tv_item_wifi_status,"("+item.getState()+")");
                helper.setText(R.id.tv_item_wifi_level,item.getLevel()+"");

                //可以传递给adapter的数据都是经过处理的，已连接或者正在连接状态的wifi都是处于集合中的首位，所以可以写出如下判断
                if( AppContants.WIFI_STATE_ON_CONNECTING.equals(item.getState()) || AppContants.WIFI_STATE_CONNECT.equals(item.getState())){
                    helper.setTextColor(R.id.tv_item_wifi_name,getResources().getColor(R.color.homecolor1));
                    helper.setTextColor(R.id.tv_item_wifi_name,getResources().getColor(R.color.homecolor1));
                }else{
                    helper.setTextColor(R.id.tv_item_wifi_name,getResources().getColor(R.color.gray_home));
                    helper.setTextColor(R.id.tv_item_wifi_name,getResources().getColor(R.color.gray_home));
                }
            }
        };
    ...
    public class WifiBroadcastReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            if(WifiManager.WIFI_STATE_CHANGED_ACTION.equals(intent.getAction())){
                ...
                Log.d(TAG,"网络列表变化了");
                if (WifiSupport.isOpenWifi(MainActivity.this) && realWifiList.size() ==0){
                    wifiListChange();
                }
            }
        }
    ...
    public void sortScaResult(){
        ...
        realWifiList.clear();
        Log.d(TAG, "sortScaResult: "+scanResults.size());
